...
namespace CleanArchitecture.ApiShared.Api
{
    public class BaseApi<T, TValue> : IApi<T, TValue>
    {
        ...
        public virtual async Task<Page<T>> GetAsync(SearchRequest searchParameters)
        {
            var url = $"{BaseUrl}/Get";
            url = GetUrl(url);
            try
            {
                HttpContent contentPost = new StringContent(JsonSerializer.Serialize(searchParameters), Encoding.UTF8, "application/json");
                var request = new HttpRequestMessage(HttpMethod.Get, url)
                {
                    Content = contentPost
                };
                AddAuthentication();
                var response = await _customHttpClient.Client.SendAsync(request);
                if (response.StatusCode == HttpStatusCode.OK)
                {
                    var json = await response.Content.ReadAsStringAsync();
                    var list = new Page<T>();
